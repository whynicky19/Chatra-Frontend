...
export const useAuthSvc=()=>{
  const api=useApi()
  return{
    login:async(email:string,pw:string)=>{const f=new URLSearchParams();f.append('username',email);f.append('password',pw);const{data}=await api.post('/auth/login',f,{headers:{'Content-Type':'application/x-www-form-urlencoded'}});return data as{access_token:string,refresh_token:string}},
